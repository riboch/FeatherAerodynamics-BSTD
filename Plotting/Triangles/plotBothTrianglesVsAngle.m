function plotBothTrianglesVsAngle(BirdData,numFPA,numV)
    ...
    for b1=1:numFPA
        ...
        for b2=1:numV
            ...
            h(b2)=plot3([BirdData(a1).base(:,1);BirdData(a1).base(1,1)],...
                         [BirdData(a1).base(:,2);BirdData(a1).base(1,2)],...
                         [BirdData(a1).base(:,3);BirdData(a1).base(1,3)],'-','Color',colours(b2,:));
            plot3(BirdData(a1).base_cg(:,1),BirdData(a1).base_cg(:,2),BirdData(a1).base_cg(:,3),'o','Color',colours(b2,:));
            ...
            quiver3(BirdData(a1).base_cg(1)*ones(1,3),BirdData(a1).base_cg(2)*ones(1,3),BirdData(a1).base_cg(3)*ones(1,3),...
                BirdData(a1).baseframe(1,:),BirdData(a1).baseframe(2,:),BirdData(a1).baseframe(3,:),'Color',colours(b2,:))% Local frame
            quiver3(BirdData(a1).base_cg(1),BirdData(a1).base_cg(2),BirdData(a1).base_cg(3),...
                	BirdData(a1).v(1),BirdData(a1).v(2),BirdData(a1).v(3),...
                    3,'Color',colours(b2,:),'LineStyle','--','LineWidth',0.5)% Velocity attached to local frame
            quiver3(BirdData(a1).base_cg(1),BirdData(a1).base_cg(2),BirdData(a1).base_cg(3),...
            ...
            plot3([BirdData(a1).tip(:,1);BirdData(a1).tip(1,1)],...
                  ...
                  [BirdData(a1).tip(:,3);BirdData(a1).tip(1,3)],'-','Color',colours(b2,:));
            plot3(BirdData(a1).tip_cg(:,1),BirdData(a1).tip_cg(:,2),BirdData(a1).tip_cg(:,3),'o','Color',colours(b2,:));
            ...
            quiver3(BirdData(a1).tip_cg(1)*ones(1,3),BirdData(a1).tip_cg(2)*ones(1,3),BirdData(a1).tip_cg(3)*ones(1,3),...
                BirdData(a1).tipframe(1,:),BirdData(a1).tipframe(2,:),BirdData(a1).tipframe(3,:),'Color',colours(b2,:))
